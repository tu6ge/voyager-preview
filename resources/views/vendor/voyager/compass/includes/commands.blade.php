@if($artisan_output)
    <pre>
        <i class="close-output voyager-x">{{ __('voyager::compass.commands.clear_output') }}</i>
        <span class="art_out">{{ __('voyager::compass.commands.command_output') }}:</span>{{ trim(trim($artisan_output,'"')) }}
    </pre>
@endif
...
@foreach($commands as $command)
    <div class="command" data-command="{{ $command->name }}">
        ...
        <form action="{{ route('voyager.compass.post') }}" class="cmd_form" method="POST">
            ...
            <input type="button" class="btn btn-primary pull-right delete-confirm"
